%% FTN NN Training - CORRECTED VERSION
% Power normalization KALDIRILDI
% N parametrik yapıldı
...
%% ========== PARAMETERS ==========
tau = 0.7;                      % FTN compression
window_len = 31;                % window size (2L+1)
num_feedback = 4;               % DFE taps
num_neighbor = 3;               % neighbor symbols
SNR_train_dB = 8;               % training SNR
N_symbols = 100000;             % training symbols
teacher_forcing_ratio = 0.7;

% CRITICAL: Power normalization flag
USE_POWER_NORM = false;  % SET TO FALSE FOR CONSISTENCY

fprintf('=== Training Models for τ=%.1f ===\n', tau);
fprintf('Power Normalization: %s\n\n', string(USE_POWER_NORM));

%% ========== TRAIN 7 MODELS ==========

% Model 1: Single
fprintf('Model 1: Single\n');
[X1, Y1] = generate_data(N_symbols, tau, SNR_train_dB, 1, 0, 0, 0, false, USE_POWER_NORM);
net1 = train_model(X1, Y1);

% Model 2: Single+DF
fprintf('Model 2: Single+DF\n');
[X2, Y2] = generate_data(N_symbols, tau, SNR_train_dB, 1, num_feedback, teacher_forcing_ratio, 0, false, USE_POWER_NORM);
net2 = train_model(X2, Y2);

% Model 3: Neighbors
fprintf('Model 3: Neighbors\n');
[X3, Y3] = generate_data(N_symbols, tau, SNR_train_dB, 1, 0, 0, num_neighbor, true, USE_POWER_NORM);
net3 = train_model(X3, Y3);

% Model 4: Neighbors+DF
fprintf('Model 4: Neighbors+DF\n');
[X4, Y4] = generate_data(N_symbols, tau, SNR_train_dB, 1, num_feedback, teacher_forcing_ratio, num_neighbor, true, USE_POWER_NORM);
net4 = train_model(X4, Y4);

% Model 5: Window
fprintf('Model 5: Window\n');
[X5, Y5] = generate_data(N_symbols, tau, SNR_train_dB, window_len, 0, 0, 0, false, USE_POWER_NORM);
net5 = train_model(X5, Y5);

% Model 6: Window+DF
fprintf('Model 6: Window+DF\n');
[X6, Y6] = generate_data(N_symbols, tau, SNR_train_dB, window_len, num_feedback, teacher_forcing_ratio, 0, false, USE_POWER_NORM);
net6 = train_model(X6, Y6);

% Model 7: Full
fprintf('Model 7: Full\n');
[X7, Y7] = generate_data(N_symbols, tau, SNR_train_dB, window_len, num_feedback, teacher_forcing_ratio, num_neighbor, false, USE_POWER_NORM);
net7 = train_model(X7, Y7);

%% ========== SAVE ==========
if ~exist('mat/comparison', 'dir'), mkdir('mat/comparison'); end
fname = sprintf('mat/comparison/tau%02d_nonorm.mat', tau*10);
save(fname, 'net1', 'net2', 'net3', 'net4', 'net5', 'net6', 'net7', ...
    'tau', 'window_len', 'num_feedback', 'num_neighbor', 'USE_POWER_NORM');
fprintf('\nSaved: %s\n', fname);

%% ========== FUNCTIONS ==========

function [X, Y] = generate_data(N, tau, SNR_dB, win_len, num_fb, tf_ratio, num_nb, neighbors_only, use_norm)
    sps = 10; span = 6;
    ...
    % CRITICAL: Noise variance calculation
    % For BPSK with unit energy symbols: Eb = 1
    % noise_var = N0/2 = 1/(2*Eb/N0) = 1/(2*10^(SNR_dB/10))
    ...
    % CRITICAL: Power normalization (CONTROLLED)
    if use_norm
        rx = rx / std(rx);
    end
